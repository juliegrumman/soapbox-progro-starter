  ...
  while (iterations < MAX_ITERATIONS) {
    ...
    if (response.stop_reason === "end_turn") {
      const text = response.content
        ...
        .map((b) => b.text)
        .join("\n");
      console.log(`   ✅ ${config.name} complete (${iterations} iteration${iterations > 1 ? "s" : ""})`);
      return text;
    }
...
async function main() {
  ...
  const [perfResult, seoResult, croResult] = await Promise.all([
    runSubAgent(buildPerfAgent()),
    runSubAgent(buildSeoMessagingAgent()),
    runSubAgent(buildCroUxAgent()),
  ]);
